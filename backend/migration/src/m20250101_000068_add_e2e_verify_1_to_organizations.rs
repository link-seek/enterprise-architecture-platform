...
/// Adds a nullable `e2e_verify_1` column to the `organizations` table.
///
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        db.execute_unprepared(
            r#"ALTER TABLE "organizations" ADD COLUMN "e2e_verify_1" TEXT"#,
        )
        .await?;
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        db.execute_unprepared(
            r#"ALTER TABLE "organizations" DROP COLUMN "e2e_verify_1""#,
        )
        .await?;
        Ok(())
    }
