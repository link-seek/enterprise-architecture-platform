...
//! the `pipeline_test_1787543819` column added by `m20250101_000061` exists,
...
#[tokio::test]
async fn migrator_creates_organizations_with_nullable_pipeline_test_1787543819() {
    ...
    .await
    .expect("insert row without pipeline_test_1787543819");

    // The omitted column should read back as NULL.
    let row = db
        .query_one_raw(sea_orm_migration::sea_orm::Statement::from_string(
            backend,
            r#"SELECT "pipeline_test_1787543819" FROM "organizations" WHERE "id" = '00000000-0000-0000-0000-000000000007'"#
                .to_string(),
        ))
        .await
        .expect("select")
    ...
    let value: Option<String> = row.try_get("", "pipeline_test_1787543819").expect("get col");
    assert!(
        value.is_none(),
        "pipeline_test_1787543819 should default to NULL when omitted"
    );

    // Updating the column to a non-null value should also work.
    db.execute_raw(sea_orm_migration::sea_orm::Statement::from_string(
        backend,
        r#"UPDATE "organizations" SET "pipeline_test_1787543819" = 'test-value' WHERE "id" = '00000000-0000-0000-0000-000000000007'"#
            .to_string(),
    ))
    .await
    .expect("update pipeline_test_1787543819");
    ...
    let value: String = row.try_get("", "pipeline_test_1787543819").expect("get col");
