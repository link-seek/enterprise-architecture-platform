...
test.describe('Space Member Management', () => {
  test('Owner can open members dialog and see member list', { tag: '@regression' }, async ({ page }) => {
    ...
    // Use .first() because the workspace may have multiple owners [e.g. the
    // seeded admin and the e2e case user are both owners in dev/CI].
  ...
  test('Owner can remove non-owner member', { tag: '@regression' }, async ({ page }) => {
    ...
    // Remove the member added by the previous case - stranger, never the
